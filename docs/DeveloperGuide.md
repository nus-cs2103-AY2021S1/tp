...
Step 2. The user executes `delete 5` command to delete the 5th stock in the location book. The `delete` command calls `Model#commitAddressBook()`, causing the modified state of the location book after the `delete 5` command executes to be saved in the `addressBookStateList`, and the `currentStatePointer` is shifted to the newly inserted location book state.

![UndoRedoState1](images/UndoRedoState1.png)

Step 3. The user executes `add n/David …​` to add a new stock. The `add` command also calls `Model#commitAddressBook()`, causing another modified location book state to be saved into the `addressBookStateList`.
