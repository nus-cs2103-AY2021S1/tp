...
1. We end up in `LogicManager#execute()`. Let’s take a look at the body of the method and annotate what we can deduce.
   ...
   **LogicManager\#execute().**
   ...
   public CommandResult execute(String commandText)
        ...
        Command command = inventoryinatorParser.parseCommand(commandText);
        ...
        try {
            ...
            storage.saveInventoryinator(model.getInventoryinator());
...
1. `Step into` the line where user input in parsed from a String to a Command.
    ...
    **`InventoryinatorParser\#parseCommand()`**
...
1. Stepping into the `switch`, we obviously stop at **`InventoryinatorParser\#parseCommand()`.**
...
1. We can a closer look at how storage works by repeatedly stepping into the code until we arrive at
    `JsonInventoryinator#saveInventoryinator()`.

1. Again, it appears that the heavy lifting is delegated. Let’s take a look at `JsonSerializableInventoryinator`'s constructor.

    **`JsonSerializableInventoryinator\#JsonSerializableInventoryinator()`:**

   ``` java
   /**
    * Converts a given {@code ReadOnlyInventoryinator} into this class for Jackson use.
    *
    * @param source future changes to this will not affect the created
    * {@code JsonSerializableInventoryinator}.
    */
   public JsonSerializableInventoryinator(ReadOnlyInventoryinator source) {
       persons.addAll(
...
1. It appears that a `JsonAdaptedPerson` is created for each `Person` and then added to the `JsonSerializableInventoryinator`.
...
more about the inner workings of Inventoryinator and how the various
...
2.  What components will you have to modify to perform the following
    ...
    3.  Save the inventory in the CSV format instead

    4.  Add a new command

    5.  Add a new field to `Person`

    6.  Add a new entity to the inventory
