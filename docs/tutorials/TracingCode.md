...
1. Now let’s see what happens when we call `command#execute()`\!
   ...
   **`EditCommand\#execute()`:**
   ...
   public CommandResult execute(Model model) throws CommandException {
       ...
       Person editedStock = createEditedPerson(stockToEdit, editPersonDescriptor);
       if (!stockToEdit.isSamePerson(editedStock) && model.hasPerson(editedStock)) {
           throw new CommandException(MESSAGE_DUPLICATE_PERSON);
       }
       model.setPerson(stockToEdit, editedStock);
       model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);
       return new CommandResult(String.format(MESSAGE_EDIT_PERSON_SUCCESS, editedStock));
   }
   ```
...
2.  What components will you have to modify to perform the following
    ...
    3.  Save the stock book in the CSV format instead

    4.  Add a new command

    5.  Add a new field to `Stock`

    6.  Add a new entity to the stock book
