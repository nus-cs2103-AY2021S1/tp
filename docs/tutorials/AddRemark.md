...
This command allows users of the AddressBook application to add optional remarks to people in their classVenue book and edit it if required. The command should have the following format:
...
Let’s start by creating a new `RemarkCommand` class in the `src/main/java/seedu/classVenue/logic/command` directory.
...
``` java
package seedu.classVenue.logic.commands;

import seedu.classVenue.model.Model;

/**
 * Changes the remark of an existing student in the classVenue book.
 */
...
``` java
import static seedu.classVenue.commons.util.CollectionUtil.requireAllNonNull;
//...
public class RemarkCommand extends Command {
    //...
...
Create a `RemarkCommandParser` class in the `seedu.classVenue.logic.parser` package. The class must extend the `Parser` interface.
...
Create a new `Remark` in `seedu.classVenue.model.student`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Simply add the following to [`seedu.classVenue.ui.PersonCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
...
:exclamation: You must delete AddressBook’s storage file located at `/data/addressbook.json` before running it! Not doing so will cause AddressBook to default to an empty classVenue book!
