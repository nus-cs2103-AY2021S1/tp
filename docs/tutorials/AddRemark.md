...
/**
 * Changes the remark of an existing stock in the location book.
 */
...
Create a new `Remark` in `seedu.location.model.stock`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
