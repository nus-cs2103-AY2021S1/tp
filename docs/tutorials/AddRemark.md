...
This command allows users of the AddressBook application to add optional remarks to people in their eva database and edit it if required. The command should have the following format:
...
:exclamation: You must delete AddressBook’s storage file located at `/data/personDatabase.json` before running it! Not doing so will cause AddressBook to default to an empty eva database!
