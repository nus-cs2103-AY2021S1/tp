...
This command allows users of the Eva application to add optional remarks to people in their eva database and edit it if required. The command should have the following format:
...
/**
 * Changes the remark of an existing person in the eva database.
 */
...
Now that we have our `RemarkCommand` ready to be executed, we need to update `EvaParser#parseCommand()` to recognize the `remark` keyword. Add the new command to the `switch` block by creating a new `case` that returns a new instance of `RemarkCommand`.
...
Create a `RemarkCommandParser` class in the `com.eva.logic.parser` package. The class must extend the `Parser` interface.
...
:information_source: Don’t forget to update `EvaParser` to use our new `RemarkCommandParser`!
...
Create a new `Remark` in `com.eva.model.person`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Simply add the following to [`com.eva.ui.PersonCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
...
You probably need to change `Staff` and `Applicant`
...
Eva stores data by serializing `JsonAdaptedStaff` or `JsonAdaptedApplicant` into `json` with the help of an external library — Jackson. Let’s update `JsonAdaptedPerson` to work with our new `Staff` or `Applicant`!

While the changes to code may be minimal, the test data will have to be updated as well.

<div markdown="span" class="alert alert-warning">

:exclamation: You must delete Eva’s storage file located at `/data` before running it! Not doing so will cause Eva to default to an empty eva database!
...
This concludes the tutorial for adding a new `Command` to Eva.
