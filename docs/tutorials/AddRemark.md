...
/**
 * Changes the remark of an existing log in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the log identified "
            + "by the index number used in the last log listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the log in the filtered log list to edit the remark
     * @param remark of the log to be updated to
     */
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of log data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the log’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a log.

### Add a new `Remark` class

Create a new `Remark` in `seedu.address.model.log`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each log.
...
``` java
public PersonCard(Person log, int displayedIndex) {
    //...
    remark.setText(log.getRemark().value);
}
```
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person logToEdit = lastShownList.get(index.getZeroBased());
        Person editedLog = new Person(logToEdit.getName(), logToEdit.getPhone(), logToEdit.getEmail(),
                logToEdit.getAddress(), remark, logToEdit.getTags());

        model.setPerson(logToEdit, editedLog);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedLog));
    }
     ...
     * {@code logToEdit}.
     */
    private String generateSuccessMessage(Person logToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, logToEdit);
    }
```
