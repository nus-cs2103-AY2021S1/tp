...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of project data. We achieve that by working with the `Project` model. Each field in a Project is implemented as a separate class (e.g. a `Name` object represents the project’s projectName). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a project.
