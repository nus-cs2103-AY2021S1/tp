...
This command allows users of the AddressBook application to add optional remarks to people in their stock book and edit it if required. The command should have the following format:
...
Let’s start by creating a new `RemarkCommand` class in the `src/main/java/seedu/stock/logic/command` directory.
...
/**
 * Changes the remark of an existing stock in the stock book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the stock identified "
            + "by the index number used in the last stock listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the stock in the filtered stock list to edit the remark
     * @param remark of the stock to be updated to
     */
...
Create a `RemarkCommandParser` class in the `seedu.stock.logic.parser` package. The class must extend the `Parser` interface.
...
Create a new `Remark` in `seedu.stock.model.stock`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each stock.

Simply add the following to [`seedu.stock.ui.PersonCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
...
StockBook stores data by serializing `JsonAdaptedPerson` into `json` with the help of an external library — Jackson. Let’s update `JsonAdaptedPerson` to work with our new `Person`!

While the changes to code may be minimal, the test data will have to be updated as well.

<div markdown="span" class="alert alert-warning">

:exclamation: You must delete StockBook’s storage file located at `/data/stockbook.json` before running it! Not doing so will cause AddressBook to default to an empty stock book!
...
``` java
public PersonCard(Person stock, int displayedIndex) {
    //...
    remark.setText(stock.getRemark().value);
}
```
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person stockToEdit = lastShownList.get(index.getZeroBased());
        Person editedStock = new Person(stockToEdit.getName(), stockToEdit.getPhone(), stockToEdit.getEmail(),
                stockToEdit.getAddress(), remark, stockToEdit.getTags());

        model.setPerson(stockToEdit, editedStock);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedStock));
    }
     ...
     * {@code stockToEdit}.
     */
    private String generateSuccessMessage(Person stockToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, stockToEdit);
    }
```
