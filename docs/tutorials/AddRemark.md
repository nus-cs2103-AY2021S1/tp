...
Create a `RemarkCommandParser` class in the `seedu.expense.logic.parser` package. The class must extend the `Parser` interface.
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of expense data. We achieve that by working with the `Expense` model. Each field in an Expense is implemented as a separate class (e.g. a `Name` object represents the expense’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a expense.

### Add a new `Remark` class

Create a new `Remark` in `seedu.expense.model.expense`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each expense.

Simply add the following to [`seedu.expense.ui.ExpenseCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
...
Then insert the following into [`main/resources/view/ExpenseListCard.fxml`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-12580431f55d7880578aa4c16f249e71).

**`ExpenseListCard.fxml`:**
...
## Modify `Expense` to support a `Remark` field

Since `ExpenseCard` displays data from a `Expense`, we need to update `Expense` to get our `Remark` displayed!

### Modify `Expense`

We change the constructor of `Expense` to take a `Remark`. We will also need to define new fields and accessors accordingly to store our new addition.

### Update other usages of `Expense`

Unfortunately, a change to `Expense` will cause other commands to break, you will have to modify these commands to use the updated `Expense`!

<div markdown="span" class="alert alert-primary">

:bulb: Use the `Find Usages` feature in IntelliJ IDEA on the `Expense` class to find these commands.
...
AddressBook stores data by serializing `JsonAdaptedExpense` into `json` with the help of an external library — Jackson. Let’s update `JsonAdaptedExpense` to work with our new `Expense`!
...
Now that we have finalized the `Expense` class and its dependencies, we can now bind the `Remark` field to the UI.
...
In this last step, we modify `RemarkCommand#execute()` to change the `Remark` of a `Expense`. Since all fields in a `Expense` are immutable, we create a new instance of a `Expense` with the values that we want and
save it with `Model#setExpense()`.
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Expense editedExpense = new Expense(expenseToEdit.getName(), expenseToEdit.getPhone(), expenseToEdit.getEmail(),
                expenseToEdit.getAddress(), remark, expenseToEdit.getTags());
