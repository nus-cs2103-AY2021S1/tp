...
This command allows users of the AddressBook application to add optional remarks to people in their fileAddress book and edit it if required. The command should have the following format:
...
Let’s start by creating a new `RemarkCommand` class in the `src/main/java/seedu/fileAddress/logic/command` directory.
...
``` java
package seedu.fileAddress.logic.commands;

import seedu.fileAddress.model.Model;

/**
 * Changes the remark of an existing description in the fileAddress book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the description identified "
            + "by the index number used in the last description listing. "
...
``` java
import static seedu.fileAddress.commons.util.CollectionUtil.requireAllNonNull;
//...
public class RemarkCommand extends Command {
    //...
    ...
    /**
     * @param index of the description in the filtered description list to edit the remark
     * @param remark of the description to be updated to
     */
...
Create a `RemarkCommandParser` class in the `seedu.fileAddress.logic.parser` package. The class must extend the `Parser` interface.
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of description data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the description’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a description.

### Add a new `Remark` class

Create a new `Remark` in `seedu.fileAddress.model.description`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each description.

Simply add the following to [`seedu.fileAddress.ui.PersonCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
...
:exclamation: You must delete AddressBook’s storage file located at `/data/addressbook.json` before running it! Not doing so will cause AddressBook to default to an empty fileAddress book!
...
``` java
public PersonCard(Person description, int displayedIndex) {
    //...
    remark.setText(description.getRemark().value);
}
```
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person editedTag = new Person(personToEdit.getName(), personToEdit.getPhone(),
                personToEdit.getAddress(), remark, personToEdit.getTags());

        model.setPerson(personToEdit, editedTag);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedTag));
    }
