...
In this tutorial, we’ll do exactly just that and remove the `fileAddress` field from `Person`.
...
1. Safe delete the field `fileAddress` in `EditPersonDescriptor`.
...
1. Remove the usages of `fileAddress` and select `Do refactor` when you are done.

   <div markdown="span" class="alert alert-primary">

   :bulb: **Tip:** Removing usages may result in errors. Exercise discretion and fix them. For example, removing the `fileAddress` field from the `Person` class will require you to modify its constructor.
   </div>
...
Unfortunately, there are usages of `Address` that IntelliJ IDEA cannot identify. You can find them by searching for instances of the word `fileAddress` in your code (`Edit` \> `Find` \> `Find in path`).

Places of interest to look out for would be resources used by the application. `main/resources` contains images and `fxml` files used by the application and `test/resources` contains test data. For example, there is a `$fileAddress` in each `PersonCard` that has not been removed nor identified.

![$fileAddress](../images/remove/$fileAddress.png)
...
In `src/test/data/`, data meant for testing purposes are stored. While keeping the `fileAddress` field in the json files does not cause the tests to fail, it is not good practice to let cruft from old features accumulate.
...
You can go through each individual `json` file and manually remove the `fileAddress` field.
