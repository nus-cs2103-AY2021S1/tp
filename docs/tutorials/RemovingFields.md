...
In this tutorial, we’ll do exactly just that and remove the `dueDate` field from `Project`.
...
The `dueDate` field in `Project` is actually an instance of the `seedu.address.model.project.DueDate` class. Since removing the `Address` class will break the application, we start by identifying `Address`'s usages. This allows us to see code that depends on `Address` to function properly and edit them on a case-by-case basis. Right-click the `Address` class and select `Refactor` \> `Safe Delete` through the menu.
...
1. Safe delete the field `dueDate` in `EditProjectDescriptor`.
...
1. Remove the usages of `dueDate` and select `Do refactor` when you are done.

   <div markdown="span" class="alert alert-primary">

   :bulb: **Tip:** Removing usages may result in errors. Exercise discretion and fix them. For example, removing the `dueDate` field from the `Project` class will require you to modify its constructor.
   </div>
...
Unfortunately, there are usages of `Address` that IntelliJ IDEA cannot identify. You can find them by searching for instances of the word `dueDate` in your code (`Edit` \> `Find` \> `Find in path`).

Places of interest to look out for would be resources used by the application. `main/resources` contains images and `fxml` files used by the application and `test/resources` contains test data. For example, there is a `$dueDate` in each `ProjectCard` that has not been removed nor identified.

![$dueDate](../images/remove/$dueDate.png)
...
``` xml
...
<Label fx:id="leader" styleClass="cell_small_label" text="\$leader" />
...
In `src/test/data/`, data meant for testing purposes are stored. While keeping the `dueDate` field in the json files does not cause the tests to fail, it is not good practice to let cruft from old features accumulate.
...
```json
{
  "projects": [ {
    ...
    "leader": "9482424",
    "projectDescription": "hans@example.com",
    "dueDate": "4th street"
  } ]
}
```

You can go through each individual `json` file and manually remove the `dueDate` field.
