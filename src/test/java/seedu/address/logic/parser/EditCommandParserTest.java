...
public class EditCommandParserTest {
    ...
    public void parse_invalidValue_failure() {
        ...
        assertParseFailure(parser, "1" + INVALID_ALLERGY_DESC, Allergy.MESSAGE_CONSTRAINTS); // invalid tag
        assertParseFailure(parser, "1" + INVALID_URL_DESC, MedicalRecord.MESSAGE_CONSTRAINTS); // invalid url
        ...
        assertParseFailure(parser, "1" + ALLERGY_DESC_NUT + ALLERGY_DESC_POLLEN
                + TAG_EMPTY, Allergy.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, "1" + ALLERGY_DESC_NUT + TAG_EMPTY
                + ALLERGY_DESC_POLLEN, Allergy.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, "1" + TAG_EMPTY + ALLERGY_DESC_NUT
                + ALLERGY_DESC_POLLEN, Allergy.MESSAGE_CONSTRAINTS);
    ...
    public void parse_allFieldsSpecified_success() {
        ...
        String userInput = targetIndex.getOneBased() + PHONE_DESC_BOB + ALLERGY_DESC_POLLEN
                + EMAIL_DESC_AMY + ADDRESS_DESC_AMY + NAME_DESC_AMY + ALLERGY_DESC_NUT + URL_DESC_AMY;

        EditCommand.EditPatientDescriptor descriptor = new EditPatientDescriptorBuilder().withName(VALID_NAME_AMY)
                .withPhone(VALID_PHONE_BOB).withEmail(VALID_EMAIL_AMY).withAddress(VALID_ADDRESS_AMY)
                .withTags(VALID_ALLERGY_POLLEN, VALID_ALLERGY_NUT).withMedicalRecord(VALID_URL_AMY).build();
    ...
    public void parse_oneFieldSpecified_success() {
        ...
        descriptor = new EditPatientDescriptorBuilder().withMedicalRecord(VALID_URL_AMY).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);

        // tags
        userInput = targetIndex.getOneBased() + ALLERGY_DESC_NUT;
        descriptor = new EditPatientDescriptorBuilder().withTags(VALID_ALLERGY_NUT).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);
    }
    ...
    public void parse_multipleRepeatedFields_acceptsLast() {
        ...
        String userInput = targetIndex.getOneBased() + PHONE_DESC_AMY + ADDRESS_DESC_AMY + EMAIL_DESC_AMY
                + ALLERGY_DESC_NUT + PHONE_DESC_AMY + ADDRESS_DESC_AMY + EMAIL_DESC_AMY + ALLERGY_DESC_NUT
                + PHONE_DESC_BOB + ADDRESS_DESC_BOB + EMAIL_DESC_BOB + ALLERGY_DESC_POLLEN + URL_DESC_BOB;

        EditCommand.EditPatientDescriptor descriptor = new EditPatientDescriptorBuilder().withPhone(VALID_PHONE_BOB)
                .withEmail(VALID_EMAIL_BOB).withAddress(VALID_ADDRESS_BOB)
                .withTags(VALID_ALLERGY_NUT, VALID_ALLERGY_POLLEN).withMedicalRecord(VALID_URL_BOB).build();
        EditCommand expectedCommand = new EditCommand(targetIndex, descriptor);
    ...
    public void parse_invalidValueFollowedByValidValue_success() {
        ...
        EditCommand.EditPatientDescriptor descriptor = new EditPatientDescriptorBuilder().withPhone(VALID_PHONE_BOB).build();
        EditCommand expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);

        // other valid values specified
        userInput = targetIndex.getOneBased() + EMAIL_DESC_BOB + INVALID_PHONE_DESC + ADDRESS_DESC_BOB
                + PHONE_DESC_BOB + URL_DESC_BOB;
        descriptor = new EditPatientDescriptorBuilder().withPhone(VALID_PHONE_BOB).withEmail(VALID_EMAIL_BOB)
                .withAddress(VALID_ADDRESS_BOB).withMedicalRecord(VALID_URL_BOB).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);
    }
