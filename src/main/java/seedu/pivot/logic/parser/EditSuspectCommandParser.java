...
public class EditSuspectCommandParser implements Parser<EditSuspectCommand> {
    @Override
    public EditSuspectCommand parse(String args) throws ParseException {
        requireNonNull(args);
        ...
        if (!matcher.matches()) {
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT,
                    EditSuspectCommand.MESSAGE_USAGE));
        }
        final String index = matcher.group("commandWord");
        final String arguments = matcher.group("arguments");

        // convert index to Index class
        Index suspectIndex = ParserUtil.getParsedIndex(index, EditSuspectCommand.MESSAGE_USAGE);
