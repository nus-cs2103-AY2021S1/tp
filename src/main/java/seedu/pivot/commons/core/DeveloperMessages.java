package seedu.pivot.commons.core;

/**
 * Container for developer visible messages.
 */
public class DeveloperMessages {

    // Assertion messages
    public static final String ASSERT_CASE_PAGE = "Program should be at case page";
    public static final String ASSERT_MAIN_PAGE = "Program should be at case page";
    public static final String ASSERT_VALID_INDEX = "Index should be valid";
    public static final String ASSERT_FILE_EXIST = "File should exist";
    public static final String ASSERT_DEFAULT_SECTION = "Program should be at default section, "
            + "listing from default cases only";
    public static final String ASSERT_ARCHIVED_SECTION = "Program should be at archived section, "
            + "listing from archived cases only";
}
