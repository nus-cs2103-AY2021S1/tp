...
public class ItemDeleteCommandParser implements Parser<ItemDeleteCommand> {
     ...
     * and returns a ItemDeleteCommand object for execution.
     * @throws ParseException if the user input does not conform the expected format
     */
    public ItemDeleteCommand parse(String args) throws ParseException {
        try {
            Index index = ParserUtil.parseIndex(args);
            return new ItemDeleteCommand(index);
        } catch (ParseException pe) {
            throw new ParseException(
                    String.format(MESSAGE_INVALID_COMMAND_FORMAT, ItemDeleteCommand.MESSAGE_USAGE), pe);
        }
    }
