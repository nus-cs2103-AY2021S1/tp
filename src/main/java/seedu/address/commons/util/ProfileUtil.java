...
public class ProfileUtil {
     ...
     */
    public static String stringifyTags(Set<Allergy> tagSet) {
        requireNonNull(tagSet, "Set<Tag> cannot be null.");

        StringBuilder sb = new StringBuilder();

        if (tagSet.size() > 0) {
            for (Allergy tag : tagSet) {
                sb.append(tag.allergyName);
                sb.append("; ");
            }
        } else {
            return "-";
    ...
    /**
     * Returns the {@code String} representation of a {@code IcNumber} instance.
     *
    ...
    /**
     * Returns the {@code String} representation of a {@code BloodType} instance.
     *
     * @param bloodType {@code IcNumber} instance to be shown as a {@code String}. Cannot be null
     * @return a {@code String} representing the {@code BloodType} instance
     */
    public static String stringifyBloodType(BloodType bloodType) {
        requireNonNull(bloodType, "Address cannot be null.");
        return bloodType.type;
    }

    /**
     * Returns the {@code String} representation of a {@code Sex} instance.
     *
     * @param sex {@code Sex} instance to be shown as a {@code String}. Cannot be null
     * @return a {@code String} representing the {@code Sex} instance
     */
    public static String stringifySex(Sex sex) {
        requireNonNull(sex, "Address cannot be null.");
        return sex.value;
    }
     ...
     */
    public static String stringifyVisit(VisitHistory visitList) {
        if (visitList == null) {
            return "-";
        }

        // Get each all VisitReport instances
        ArrayList<Visit> visits = visitList.getVisits();
     ...
     */
    public static String stringifyVisitReport(Visit report) {
        requireNonNull(report, "VisitReport cannot be null.");

        String date = report.getVisitDate().toString();
        String diagnosis = report.getDiagnosis();
        String medication = report.getPrescription();
        String remarks = report.getComment();
