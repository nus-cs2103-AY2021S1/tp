...
public class TaskDashboard extends UiPart<Region> {
     ...
     */
    public TaskDashboard(Optional<Project> project) {
        super(FXML);
        this.task = project.get().getTaskOnView().get();
        taskName.setText(this.task.getTaskName());
        taskDescription.setText("Task description: " + this.task.getDescription());
        publishDate.setText("Task published date: " + this.task.getPublishDateInString());
        deadline.setText("Task deadline: " + this.task.getDeadline().toString());
        progress.setText("Task progress: " + this.task.getProgress());
        header1.setText("Task assignees: ");
        this.task.getAssignees()
                .forEach(participation -> assignees.getChildren().add(new Label(participation.getPerson().getPersonName().toString())));
    }
